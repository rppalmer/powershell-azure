...
$ResourceGroup     	= "xxx"
$AutomationAcctName	= "xxx"
$Lookback           = 1

$Jobs = Get-AzAutomationJob -ResourceGroupName $ResourceGroup -AutomationAccountName $AutomationAcctName | `
Where-Object {$_.status -ne "Completed" -and $_.status -ne "Running" -and $_.status -ne "Activating" -and $_.status -ne "New" -and $_.LastModifiedTime -gt (get-date).adddays(-$Lookback)} | `
...
if ($Jobs)
{
    ...
    Start-AzAutomationRunbook –Name 'Set-SendGridMessagev2' -AutomationAccountName $AutomationAcctName -ResourceGroupName $ResourceGroup –Parameters $params
